...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            with adapter_ctx:
                output, entropys, expert_logprobs = self.actor.compute_log_prob(data=data, calculate_entropy=True)
            if expert_logprobs is not None:
                output = DataProto.from_dict(
                    tensors={"old_log_probs": output, "entropys": entropys, "old_expert_logprobs": expert_logprobs},
