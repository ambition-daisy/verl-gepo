...
@register_policy_loss("gepo")  # type: ignore[arg-type]
def compute_policy_loss_gepo(
    ...
    negative_approx_kl = log_prob - old_log_prob # B
    # Clamp negative_approx_kl for stability
    negative_approx_kl = torch.clamp(negative_approx_kl, min=-20.0, max=20.0)
    ratio = torch.exp(negative_approx_kl)
    ppo_kl = -negative_approx_kl.mean()

    advantages = advantages.sum(dim=-1) / response_mask.sum(dim=-1) # B

    if log_prob.dim() == 3:
        advantages = advantages.unsqueeze(-1).unsqueeze(-1).expand(-1,log_prob.shape[1],log_prob.shape[2])
    ...
    return pg_loss, torch.tensor(0.0).to(pg_loss.device), ppo_kl, torch.tensor(0.0).to(pg_loss.device)
